...
const ADMIN_IDS = (process.env.ADMIN_IDS || "867717817")
  .split(",")
  .map((x) => x.trim())
  .filter(Boolean)
  .map(Number);
...
async function showMainMenu(ctx) {
  ...
  // START paytida telefon raqamini qabul qilish
  if (state.state === "START_PHONE") {
    ...
    db.prepare(`
      ...
      SET phone = ?
      WHERE telegram_id = ?
    `).run(phone, ctx.from.id);

    clearState(ctx.from.id);

    // USERGA TELEFON QABUL QILINGANI
    await ctx.reply(
    ...
    await sendToAdmins(
      ...
      `🔗 <b>Username:</b> @${escapeHtml(ctx.from.username || "username yo‘q")}\n` +
      `🆔 <b>Telegram ID:</b> ${ctx.from.id}\n` +
      `📱 <b>Telefon:</b> ${escapeHtml(phone)}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━`
    );

    // SENING MAVJUD A-FARID WELCOME XABARING
    await ctx.reply(
      `<b>✨ A-FARID STYLE TURKEY</b>\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━━\n\n` +
...
bot.on("text", async (ctx, next) => { 
  ...
  if (state.state === "ORDER_PHONE") { 
    ...
    await ctx.reply( 
      "📦 <b>NECHA POCHKA?</b>\n\nNecha pochka zakaz qilmoqchisiz?", 
      { 
        parse_mode: "HTML", 
        ...backKeyboard(), 
      } 
